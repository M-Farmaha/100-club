...
export const AdminStagePage = () => {
  ...
  const handleSaveStage = (updatedStage) => {
    ...
    if (isNew) {
      showMessage("✅ Новий етап додано");
      setTimeout(() => {
        navigate(`/admin/${tournamentId}`);
      }, 1000);
      ...
      navigate(`/admin/${tournamentId}/${year}/${updatedStage.date}`, { 
        replace: true,
  ...
  return (
    <AdminContainer>
      <BackLink onClick={() => navigate(`/admin/${tournamentId}`)}>
        <svg>
